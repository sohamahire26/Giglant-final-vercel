            ...
            <div className={`lg:col-span-2 space-y-6 ${previewMode ? "hidden" : "block"}`}>
              <div className="rounded-2xl border border-border bg-card p-6 shadow-sm space-y-4">
                ...
                <div className="grid gap-4 sm:grid-cols-2">
                  <div className="space-y-2">
                    ...
                    <Input value={post.slug} onChange={e => setPost({...post, slug: e.target.value})} placeholder="post-url-slug" />
                  </div>
                  <div className="space-y-2">
                ...
                <div className="space-y-2">
                  <label className="text-xs font-bold uppercase tracking-widest text-muted-foreground">Excerpt (Short Summary)</label>
                  <Textarea 
                    value={post.excerpt} 
              ...
              <div className="rounded-2xl border border-border bg-card p-6 shadow-sm space-y-4">
                <div className="flex items-center justify-between">
                  <label className="text-xs font-bold uppercase tracking-widest text-muted-foreground">Content (HTML/Markdown)</label>
                  <span className="text-[10px] text-muted-foreground">Supports standard HTML tags for styling</span>
                </div>
                <Textarea 
                  value={post.content} 
            ...
            <div className={`lg:col-span-2 ${previewMode ? "block" : "hidden"}`}>
              <div className="rounded-2xl border border-border bg-card p-8 shadow-sm min-h-[700px]">
                {post.cover_image_url && (
                  <img src={post.cover_image_url} alt="Cover" className="mb-8 w-full rounded-xl object-cover aspect-video" />
                )}
                ...
                <h1 className="font-display text-4xl font-bold text-foreground mb-6">{post.title || "Post Title Preview"}</h1>
                ...
                <article className="prose prose-lg prose-red max-w-none">
                  <div className="blog-content" dangerouslySetInnerHTML={{ __html: post.content || "<p className='text-muted-foreground'>No content yet...</p>" }} />
                </article>
              </div>
            </div>
            ...
            <div className="space-y-6">
              <div className="rounded-2xl border border-border bg-card p-6 shadow-sm space-y-6">
                <div className="space-y-4">
                  ...
                  <label className="flex cursor-pointer items-center gap-3 rounded-xl border border-border p-4 transition-colors hover:bg-secondary">
                    <input 
                      type="checkbox" 
                ...
                <div className="space-y-4 pt-6 border-t border-border">
                  ...
                  <div className="space-y-3">
                    <div className="space-y-1">
                      ...
                      <Input value={post.meta_title} onChange={e => setPost({...post, meta_title: e.target.value})} placeholder="SEO Title" />
                    </div>
                    <div className="space-y-1">
                      <label className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground">Meta Description</label>
                      <Textarea value={post.meta_description} onChange={e => setPost({...post, meta_description: e.target.value})} placeholder="SEO Description" rows={3} />
                    </div>
                  </div>
                </div>
              </div>
