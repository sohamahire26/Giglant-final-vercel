...
import { MessageSquare, Copy, Mail, History, Trash2, CheckCircle2, RefreshCw, Send, Info, ArrowRight } from "lucide-react";
...
const PaymentGeneratorTool = () => {
  ...
  useEffect(() => {
    const savedHistory = localStorage.getItem("payment_message_history_standalone");
    if (savedHistory) {
      try {
  ...
  const saveToHistory = (subject: string, body: string) => {
    ...
    localStorage.setItem("payment_message_history_standalone", JSON.stringify(updatedHistory));
  };
  ...
  return (
    <Layout>
      <SEOHead title="Payment Message Generator — Giglant" description="Generate professional payment requests and reminders for your freelance clients." />
      <section className="section-padding">
        <div className="container-tight max-w-4xl">
          <div className="mb-12 text-center">
            ...
            <h1 className="font-display text-4xl font-bold text-foreground md:text-5xl">Payment Generator</h1>
            <p className="mt-4 text-lg text-muted-foreground">Generate professional payment requests and reminders using deterministic templates.</p>
          </div>

          {!session && (
            <div className="mb-12 rounded-3xl bg-foreground p-8 text-center text-background md:p-12">
              <h2 className="font-display text-2xl font-bold">Manage Full Projects</h2>
              <p className="mt-3 text-background/70">
                Sign up to save your project details, track client feedback, and manage your entire freelance workflow in one place.
              </p>
          ...
          <div className="mt-16">
            <h2 className="font-display text-2xl font-bold text-foreground mb-6">How It Works — Professional Billing</h2>
            <div className="grid gap-4 md:grid-cols-4">
              {[
                { step: "Enter Details", desc: "Fill in the client name, amount, and project deadline." },
                { step: "Select Type", desc: "Choose between initial request, reminder, or final notice." },
                { step: "Generate", desc: "Get a perfectly formatted message based on proven templates." },
                { step: "Send", desc: "Copy the text or open directly in Gmail to send to your client." },
              ].map((s, i) => (
...
export default PaymentGeneratorTool;